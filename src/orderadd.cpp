...
int* orderadd::readFloorsWithDesks(const char* fileName, int* outCount) {
    ...
    timer(new QTimer(this))
{
    ...
    QLabel *statusLabel = new QLabel("      选择至少一样菜品。多个订单的拼桌是被允许的，但无法输入比所选桌子空闲位数多的人数。                      ", this);
...
void orderadd::on_buttonBox_accepted()
{
    ...
    setDeskCurrentPeople(x,y,z,ui->countpeopleSpinBox->value());

    //向父窗口的哈希数组添加存储着新订单的新节点
    int ordercount;
...
void orderadd::addRow(char *name, char *taste, int count)
{
    ...
    dishItem->setFlags(dishItem->flags() & ~Qt::ItemIsEditable);
    tasteItem->setFlags(tasteItem->flags() & ~Qt::ItemIsEditable);
